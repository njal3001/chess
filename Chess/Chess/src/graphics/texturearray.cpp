...
namespace Game
{
    TextureArray::TextureArray(const unsigned int max_layers, const Vec2& max_size, GLuint internal_format, GLuint format, const GLuint alignment)
    ...
    const TextureArray::Element TextureArray::add(const std::string& filename)
    {
        ...
        const Vec2 uv_scale = Vec2(width / m_max_size.x, height / m_max_size.y);
